...
# Perhaps use MPI
def ns_run_batch(config_path, parallel, enb_gdb, enb_plot, upd_LHR):
  t1 = time.time()
  ...
  for instance_id in range(config["batch_size"]):
    ...
    for param in config.keys():
      ...
      # added on 08.11 to enable plotting
      if enb_plot and instance_id == 0 and param == "result_dir":
        result = "/".join(((config[param][instance_id]).split("/"))[:2])

      # added on 08.31 to consider termination time
      if enb_plot and instance_id == 0 and param == "termination_time_ms":
        termination_time = str(config[param])

      # added on 09.12 to enable sampling
      if enb_plot and instance_id == 0 and param == "sampling_num":
        sampling_num = str(config[param])

      if param != "instance_type" and param != "batch_params" and param != "batch_size" and param != "termination_time_ms" and param != "sampling_num":
  ...
  if enb_plot:
    if upd_LHR:
      os.system("python plot.py " + result + " " + termination_time + " upd_LHR" + " " + sampling_num)
    else:
      os.system("python plot.py " + result + " " + termination_time + " not_upd_LHR" + " " + sampling_num)
...
def plot_pl3(data_dir):
  ...
  num_cca = int(batch_config['num_cca'])
  if num_cca != 4:
    print("ERR: Presume 4 ccas")
    exit()
  num_cca0 = int(batch_config['num_cca0'])
  num_cca1 = int(batch_config['num_cca1'])
  num_cca2 = int(batch_config['num_cca2'])
  num_cca3 = int(batch_config['num_cca3'])  
  num_flows = num_cca0+num_cca1+num_cca2+num_cca3
  ...
  ideal_cca2_bw = 0  
  ideal_cca3_bw = 0      
  if num_cca1 >= num_cca2 and num_cca1 >= num_cca3:
    max_conn = num_cca1
    max_conn_sum = max_conn + num_cca0
    ideal_cca0_bw = 1.0*bottleneck_bw/max_conn_sum
    ideal_cca1_bw = 1.0*bottleneck_bw/max_conn_sum
    ideal_cca2_bw = (bottleneck_bw-num_cca0*ideal_cca0_bw)/num_cca2
    ideal_cca3_bw = (bottleneck_bw-num_cca0*ideal_cca0_bw)/num_cca3  
  elif num_cca2 >= num_cca1 and num_cca2 >= num_cca3:
    max_conn = num_cca2
    max_conn_sum = max_conn + num_cca0
    ideal_cca0_bw = 1.0*bottleneck_bw/max_conn_sum
    ideal_cca2_bw = 1.0*bottleneck_bw/max_conn_sum   
    ideal_cca1_bw = (bottleneck_bw-num_cca0*ideal_cca0_bw)/num_cca1
    ideal_cca3_bw = (bottleneck_bw-num_cca0*ideal_cca0_bw)/num_cca3   
  elif num_cca3 >= num_cca1 and num_cca3 >= num_cca2:
    max_conn = num_cca3
    max_conn_sum = max_conn + num_cca0
    ideal_cca0_bw = 1.0*bottleneck_bw/max_conn_sum
    ideal_cca3_bw = 1.0*bottleneck_bw/max_conn_sum   
    ideal_cca1_bw = (bottleneck_bw-num_cca0*ideal_cca0_bw)/num_cca1
    ideal_cca2_bw = (bottleneck_bw-num_cca0*ideal_cca0_bw)/num_cca2
  else:
    print("ERR")
  ...
  for i in range(num_flows):
    if i < num_cca0:
      ...
      gpts_str += (str(i)+" "+str(ideal_cca2_bw)+"\n")
    elif i < num_cca0+num_cca1+num_cca2+num_cca3:
      gpts_str += (str(i)+" "+str(ideal_cca3_bw)+"\n")      
  gpts_str += "\n\n"
  ...
  for i in range(num_flows):
    if i < num_cca0:
      ...
      fifo_gpts_norm.append(fifo_gpts[i]/ideal_cca3_bw)              
  # print(fifo_gpts)
  # print(fifo_gpts_norm)
  square_of_sums = 0
  sum_of_squares = 0
  ...
  for i in range(num_flows):
    if i < num_cca0:
      ...
      fq_gpts_norm.append(fq_gpts[i]/ideal_cca3_bw)              
  # print(fifo_gpts)
  # print(fifo_gpts_norm)
  square_of_sums = 0
  sum_of_squares = 0
  ...
  for i in range(num_flows):
    if i < num_cca0:
      ...
      cb_gpts_norm.append(cb_gpts[i]/ideal_cca3_bw)              
  # print(fifo_gpts)
  # print(fifo_gpts_norm)
  square_of_sums = 0
  sum_of_squares = 0
...
if __name__ == '__main__':
  ...
  # added on 08.08 to enable gdb
  ns_run_batch_prsr.add_argument("--gdb", action="store_true", help="Whether to enable gdb")
  # added on 08.12 to enable plotting
  ns_run_batch_prsr.add_argument("--plot", action="store_true", help="Whether to enable plotting")
  # added on 09.07 to enable lower_headroom selection
  ns_run_batch_prsr.add_argument("--newLHR", action="store_true", help="Whether to update lower_headroom")
  ...
  if args.cmd == "ns":
    ...
    if args.ns_cmd == "build":
      ...
      # edited on 08.08 to enable gdb
      # edited on 08.12 to enable plotting
      ns_run_batch(args.config, args.parallel, args.gdb, args.plot, args.newLHR)
